...
const CardItem = ({ card, index, moveCard, updateCardStatus, updateCard }) => {
  ...
  const handleSave = () => {
    updateCard(card.id, editedTitle, card.timestamp);
      ...
      >
        <div className="card">
          ...
          <div className="mt-5 mb-2 mr-3 flex justify-end">
            <div className="ml-2 text-[12px] text-gray-500">
              {card.timestamp}
            </div>
          </div>
        </div>
      ...
      {isModalOpen && (
        <Dialog open={isModalOpen} onOpenChange={() => setIsModalOpen(false)}>
          ...
          <DialogContent>
            ...
            />
            <Input type="text" value={card.timestamp} disabled />
            <div className="flex gap-5">
              <div >
              <Button className=" bg-gray-200 text-black font-semibold rounded-lg shadow-md hover:bg-blue-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-opacity-75 border border-blue-400" onClick={handleSave}>Save</Button>
              </div>
              <div><DialogClose asChild>
                <Button className=" bg-gray-200 text-black font-semibold rounded-lg shadow-md hover:bg-red-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-opacity-75 border border-red-400">Cancel</Button>
              </DialogClose></div>
            </div>
          </DialogContent>
        </Dialog>
...
const Column = ({
  ...
  const columnClasses = `border-r-2 ${
    ["Open", "Working", "Completed", "Overdue"].includes(status)
      ? "max-h-screen overflow-y-auto"
      : ""
  } ${isOverCurrent ? "bg-gray-200 " : ""}`;
